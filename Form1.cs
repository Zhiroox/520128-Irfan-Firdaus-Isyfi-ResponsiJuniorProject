...
MessageBox.Show("Aplikasi siap digunakan!", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
     }
  catch (Exception ex)
            {
...
comboBox2.SelectedIndex = 0;
   textBox1.Focus();
        ...
        /// <summary>
        /// Event handler ketika row di-klik di DataGridView
        /// </summary>
  private void DataGridView1_CellClick(object? sender, DataGridViewCellEventArgs e)
 {
            try
         {
    if (e.RowIndex < 0 || _developerService == null)
  return;

             var row = dataGridView1.Rows[e.RowIndex];
      
  // Populate textboxes
     textBox1.Text = row.Cells["NamaDev"].Value?.ToString() ?? "";
   textBox2.Text = row.Cells["FiturSelesai"].Value?.ToString() ?? "";
      textBox3.Text = row.Cells["JumlahBug"].Value?.ToString() ?? "";
   
 // Set status kontrak
  string statusKontrak = row.Cells["StatusKontrak"].Value?.ToString() ?? "";
      if (comboBox2.Items.Contains(statusKontrak))
     comboBox2.SelectedItem = statusKontrak;

      // Set proyek berdasarkan IdProyek dari row
 int idProyek = (int)(row.Cells["IdProyek"].Value ?? 0);
     if (idProyek > 0)
          {
    for (int i = 0; i < comboBox1.Items.Count; i++)
 {
   if (comboBox1.Items[i] is ProyekItem item && item.Id == idProyek)
         {
...
private void Form1_FormClosing(object? sender, FormClosingEventArgs e)
     {
         // Tidak perlu close connection - repositories handle sendiri
 }
    }
}
